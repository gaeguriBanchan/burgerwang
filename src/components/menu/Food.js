...
const Food = ({ menu }) => {
  const { name, count, uri, detail, price, best, seq, soldout } = menu;
  ...
  return (
    <>
      <div className="grid grid-rows-none" onClick={() => openModal()}>
        <img
      ...
      <OptionModal modalIsOpen={modalIsOpen} closeModal={closeModal} />
    </>
  );
};
